...
validate_public_domain() {
  ...
  if ! curl --fail --silent --show-error --location "https://${SPARKS_HOST}/healthz" > /dev/null; then
    rm -f "${body_file}"
    return 1
  fi

  if ! curl --fail --silent --show-error --location "https://${SPARKS_HOST}/" > "${body_file}"; then
    rm -f "${body_file}"
    return 1
  fi
